...
import useDocumentSearch from '@/app/hooks/useDocumentSearch';
import React from 'react';

export default function DocumentUploadPage({ params }: { params: Promise<{ id: number }> }) {
    // Unwrap the params promise
    const resolvedParams = React.use(params);
    const { data, error, isLoading } = useDocument({ documentId: resolvedParams.id });
    const { semanticSearch } = useDocumentSearch()
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen">
            ...
            <form className="relative w-full max-w-2xl rounded-lg shadow-md">
                <textarea 
                    id="hs-textarea-ex-1" 
